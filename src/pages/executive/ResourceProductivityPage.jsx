...
export default function ProductividadRecursoPage() {
  ...
  const { data: ausencias = [] } = useQuery({
    ...
    queryFn: () => ausenciaService.list({ resource_id: recursoId }),
    enabled: !!recursoId,
  })

  const recursoSel = recursos.find((r) => r.id === recursoId)
  const filtrados = recursos.filter((r) =>
    (!tipoFiltro || r.type === tipoFiltro) &&
  ...
  return (
    <div className="p-3 sm:p-4 max-w-4xl mx-auto">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between mb-4 print:hidden">
        <div>
                  ...
                  <div className="absolute z-20 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-56 overflow-y-auto py-1">
                    ...
                    {filtrados.length === 0 ? (
                      ...
                      filtrados.map((r) => (
                        ...
                        >
                          <span>{r.name}</span>
                          <span className="text-xs text-gray-400">{tipoLabel(r.type)}</span>
                        </button>
                      ))
                    )}
                  </div>
...
function Detalle({ data, resource: recurso, absences: ausencias = [] }) {
  ...
  const variacionHoras = (data.current_week_hours ?? 0) - promedio.horas
    ...
    <>
      <div className="mb-3">
        <h2 className="text-sm font-semibold text-gray-800">
          Estadísticas de {recurso?.name ?? 'recurso'}
          {recurso?.type ? <span className="text-gray-400 font-normal"> · {tipoLabel(recurso.type)}</span> : null}
        </h2>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        ...
        />
        <KpiCard label="Horas del mes" value={`${data.horas_mes ?? 0}h`} />
        <KpiCard
          label="Pacientes esta semana"
          value={data.pacientes_semana ?? 0}
          delta={variacionPac >= 0 ? `+${variacionPac} vs promedio` : `${variacionPac} vs promedio`}
          deltaUp={variacionPac >= 0}
        />
        <KpiCard label="Pacientes del mes" value={data.pacientes_mes ?? 0} />
      </div>
              ...
              <tbody>
                {ausencias.map((a) => (
                  <tr key={a.id} className="border-b border-gray-50 hover:bg-gray-50">
                    <td className="px-3 py-2 text-gray-700 whitespace-nowrap">{fmtRango(a.start_date, a.end_date)}</td>
                    <td className="px-3 py-2 text-gray-700">{ausenciaLabel(a.type)}</td>
                    <td className="px-3 py-2"><Badge variant={estadoVariant(a.status)}>{a.status}</Badge></td>
                    <td className="px-3 py-2 text-right text-gray-700">{a.patients_affected ?? '—'}</td>
                    <td className="px-3 py-2 text-right text-gray-700">{a.opportunity_cost ? formatCOP(a.opportunity_cost) : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
