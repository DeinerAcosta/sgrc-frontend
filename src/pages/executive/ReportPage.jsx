            ...
            <table className="w-full text-xs border-collapse min-w-[800px]">
              ...
              <tbody>
                {dataOrdenada.map((row, i) => {
                  ...
                  // PROYECTOS-3255 #3.1: recurso sin actividad (0h programadas) → tint gris
                  const sinActividad = tipo === 'productividad' && row.h_prog === 0
                  const rowClass = anomaliaDiasNegativos
                    ? 'border-b border-red-100 bg-red-50/40 hover:bg-red-50 transition-colors'
                    : enIncapacidad
                      ? 'border-b border-blue-100 bg-blue-50/30 hover:bg-blue-50 transition-colors'
                      : sinActividad
                        ? 'border-b border-gray-100 bg-gray-50/40 text-gray-500 hover:bg-gray-100 transition-colors'
                    ...
                    <tr key={i} className={rowClass} title={anomaliaDiasNegativos ? '⚠️ Alguna ausencia de este recurso tiene fechas corruptas (fin antes que inicio, o año mal escrito). Requiere corrección manual en el módulo de Ausencias.' : undefined}>
                      {indicesVisibles.map((origIdx, j) => {
                        ...
                        const iconoWarning = anomaliaDiasNegativos && col === 'Recurso'
                          ...
                          : null
                        // PROYECTOS-3255 #1.3 · Badge "En incapacidad" al lado del nombre
                        const badgeIncapacidad = enIncapacidad && col === 'Recurso'
                          ? <Badge variant="blue" className="ml-2" title={`Con incapacidad médica en el periodo — ${row.dias_incapacidad}d`}>🩹 Incapacidad</Badge>
                          : null
