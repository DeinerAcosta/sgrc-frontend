...
export default function AdminTareasBackofficePage() {
  ...
  const aprobadas  = tareas.filter((t) => t.estado === 'aprobada' || !t.estado)
  const rechazadas = tareas.filter((t) => t.estado === 'rechazada')
  const activas    = aprobadas.filter((t) => t.activa).length
  ...
  return (
    <div className="p-4">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-500 mt-0.5">
                      Tiempo estimado sugerido: <strong>{t.tiempo_estimado_minutos} min/unidad</strong>
                    </div>
                    {t.justificacion && (
          ...
          <div className="mt-2 space-y-1.5">
            {rechazadas.map((t) => (
              <div key={t.id} className="bg-gray-50 rounded-lg px-3 py-2">
                <div className="font-medium text-gray-700">{t.nombre}</div>
                {t.motivo_rechazo && <div className="italic">Motivo: {t.motivo_rechazo}</div>}
                <div className="text-gray-400">{format(parseISO(t.procesada_en ?? t.created_at), "d MMM yyyy", { locale: es })}</div>
              </div>
            ))}
          </div>
        </details>
      ...
      {aprobando && <AprobarModal tarea={aprobando} onClose={() => setAprobando(null)} onSaved={() => { qc.invalidateQueries(['admin-tareas-backoffice']); setAprobando(null) }} />}
      {rechazando && <RechazarModal tarea={rechazando} onClose={() => setRechazando(null)} onSaved={() => { qc.invalidateQueries(['admin-tareas-backoffice']); setRechazando(null) }} />}
    </div>
  )
}
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="px-5 py-4 space-y-4">
          {tarea.justificacion && (
            <div className="text-xs text-gray-700 italic bg-gray-50 px-3 py-2 rounded-lg">
              💬 Justificación del coordinador: {tarea.justificacion}
            </div>
          )}
          <div>
            <label className="label">Nombre *</label>
          ...
          <div>
            ...
            <textarea className="input resize-none" rows={3} value={form.descripcion} onChange={(e) => setForm({ ...form, descripcion: e.target.value })} placeholder="Describe brevemente qué incluye la tarea..." />
          </div>
          <div>
            <label className="label">Tiempo estimado por unidad (min) *</label>
            <input className="input" type="number" min="1" max="480" value={form.tiempo_estimado_minutos} onChange={(e) => setForm({ ...form, tiempo_estimado_minutos: parseInt(e.target.value) || 0 })} />
          </div>
        ...
        <div className="px-5 py-4 border-t border-gray-100 flex gap-2">
          <button className="btn flex-1 justify-center" onClick={onClose}>Cancelar</button>
          <button className="btn-success flex-1 justify-center" onClick={() => mutate()} disabled={!form.nombre || isPending}>
    ...
    <div className="fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100">
          ...
          <p className="text-xs text-gray-500 mt-0.5">Tarea: "{tarea.nombre}"</p>
        </div>
