...
export default function AdminFestivosPage() {
  ...
  const confirmarEliminar = async (f) => {
    const ok = await confirm({
      title: '¿Eliminar este festivo?',
      message: `Vas a eliminar "${f.description}" del calendario. Esta acción no se puede deshacer.`,
      confirmLabel: 'Eliminar',
      cancelLabel: 'Cancelar',
      variant: 'danger',
    })
    if (ok) eliminar(f.date)
  }
  ...
  const { mutate: eliminar } = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-festivos'] }); toast.success('Festivo eliminado') },
  })

  const { mutate: sincronizar, isPending: sincronizando } = useMutation({
    mutationFn: () => festivoService.sincronizarColombia(),
    onSuccess: (res) => {
      qc.invalidateQueries({ queryKey: ['admin-festivos'] })
  ...
  const ordenados = [...festivos].sort((a, b) => a.date.localeCompare(b.date))
  const futuros = ordenados.filter((f) => f.date >= format(new Date(), 'yyyy-MM-dd'))
  const pasados = ordenados.filter((f) => f.date < format(new Date(), 'yyyy-MM-dd'))
  ...
  return (
    <div className="p-3 sm:p-4">
        ...
        <div className="card">
          ...
          {isLoading ? (
            ...
            <div className="space-y-1">
              {futuros.map((f) => (
                <div key={f.date} className="flex items-center justify-between gap-3 py-1.5 border-b border-gray-50 last:border-0">
                  <div className="flex-1 min-w-0">
                    <div className="text-xs font-medium text-gray-800">
                      {format(parseFechaLocal(f.date), "EEEE d 'de' MMMM", { locale: es })}
                    </div>
        ...
        <div className="card">
          ...
          {pasados.length === 0 ? (
            ...
            <div className="space-y-1 max-h-96 overflow-y-auto">
              {pasados.map((f) => (
                <div key={f.date} className="flex items-center justify-between gap-3 py-1.5 border-b border-gray-50 last:border-0 opacity-70">
                  <div className="flex-1 min-w-0">
                    <div className="text-xs text-gray-700">
                      {format(parseFechaLocal(f.date), 'd MMM yyyy', { locale: es })}
                    </div>
      ...
      {showAdd && <FestivoModal onClose={() => setShowAdd(false)} onSaved={() => { qc.invalidateQueries({ queryKey: ['admin-festivos'] }); setShowAdd(false) }} />}
    </div>
  )
}
...
function FestivoModal({ onClose, onSaved }) {
  ...
  const dirtySnapshot = { date: fecha, description: descripcion }
  const { tryClose } = useDirtyClose(dirtySnapshot, onClose)

  const { mutate, isPending } = useMutation({
    mutationFn: () => festivoService.create({ date: fecha, description: descripcion }),
